...
module SearchHelper
  ...
  def active_nav?(active_scope, active_type, scope_name, type)
    work_item_scope = 'issues'

    if @scope.to_s != work_item_scope
      active_scope
    elsif type
      active_type
    else
      params[:type].nil? ? active_scope : false
    end
  end
  ...
  def search_filter_link_json(scope, label, data, search, type)
    ...
    result = {
      ...
      active: active_nav?(active_scope, active_type, scope_name, type)
    }
