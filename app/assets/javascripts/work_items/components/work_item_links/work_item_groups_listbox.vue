...
export default {
  ...
  apollo: {
    group: {
      ...
      result() {
        this.selectedGroup = this.findSelectedGroup(this.selectedGroupFullPath);
      },
    },
  },
  computed: {
  ...
  methods: {
    ...
    handleSelect(groupFullPath) {
      this.selectedGroup = this.findSelectedGroup(groupFullPath);
      this.$emit('selectGroup', groupFullPath);
    },
    findSelectedGroup(groupFullPath) {
      const group = this.filteredGroups.find((g) => g.fullPath === groupFullPath);
      if (group) {
        return group;
      }
      return this.filteredGroups.find((g) => {
        return `/${g.fullPath}` === groupFullPath;
      });
    },
