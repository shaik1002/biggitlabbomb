...
export const currentScope = (state, parentScope = null) => {
  const data = state.navigation ? state.navigation : state;
  for (const value of Object.values(data)) {
    ...
    if (value.sub_items) {
      const subItemScope = currentScope(value.sub_items, value.scope);
      if (subItemScope) {
        return subItemScope;
      }
    }
