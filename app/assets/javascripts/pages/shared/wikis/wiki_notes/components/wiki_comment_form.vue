...
export default {
  ...
  computed: {
    ...
    canCreateNote() {
      if (this.isContainerArchived) return false;
      return true;
    },
    autosaveKey() {
  ...
  methods: {
    ...
    async handleSave() {
      ...
      try {
        ...
        this.errors = createNoteErrorMessages(err);
        this.$emit('creating-note:failed', err);
        this.note = input.body;

        this.timeoutIds.push(
          setTimeout(() => {
            // delaying this because there is a chance the first timeout doesn't execute before this.
