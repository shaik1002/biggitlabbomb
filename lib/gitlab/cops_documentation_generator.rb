...
# NOTE: This code has been copied directly from Rubocop[1]
# Before merging we should determine whether we should extend the existing code, or what attribution we should include.
# [1] https://github.com/rubocop/rubocop/blob/master/lib/rubocop/cops_documentation_generator.rb
...
module Gitlab
  class CopsDocumentationGenerator # rubocop:disable Metrics/ClassLength
    ...
    #
    def initialize(departments: [], cops: [], extra_info: {}, base_dir: Dir.pwd)
      @departments = departments.map(&:to_sym).sort!
      @extra_info = extra_info

      # GitLab note: Changed to exclude existing cops, and only show GitLab cops
      @cops = RuboCop::Cop::Registry.new
      cops.each { |cop| @cops.enlist(cop) }
    ...
    def table_of_content_for_department(department)
      ...
      filename = "/rubocop/#{department_to_basename(department)}/index.adoc" # GitLab note: Changed to edit the URLs
