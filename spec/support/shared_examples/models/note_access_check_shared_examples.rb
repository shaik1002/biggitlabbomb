...
RSpec.shared_examples 'users with note access' do
  it 'returns true' do
    users.each do |user|
      expect(Notes::SystemNoteFacade.new(note).system_note_visible_for?(user)).to be_truthy
...
RSpec.shared_examples 'users without note access' do
  it 'returns false' do
    users.each do |user|
      expect(Notes::SystemNoteFacade.new(note).system_note_visible_for?(user)).to be_falsy
