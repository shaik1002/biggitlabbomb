...
RSpec.shared_examples 'Auth::AuthFinders authenticates the user' do
  ...
  describe '#find_user_from_feed_token' do
    context 'when the request format is atom' do
      ...
      context 'when path-dependent format feed_token param is provided' do
        ...
        let(:feed_token_digest) { OpenSSL::HMAC.hexdigest("SHA256", 'KNOWN VALUE', 'url.atom') }
        let(:feed_token) { "glft-#{feed_token_digest}-#{feed_user.id}" }
        ...
        it 'returns exception if token has same HMAC but different user ID' do
          feed_token = "glft-#{feed_token_digest}-#{user.id}"
          set_param(:feed_token, feed_token)
        ...
        it 'returns exception if token has wrong HMAC but same user ID' do
          feed_token = "glft-aaaaaaaaaade004d09a968705ba49099229b288b3de43f26c473a9d8d7fb7693-#{feed_user.id}"
          set_param(:feed_token, feed_token)
        ...
        it 'returns exception if user does not exist' do
          feed_token = "glft-#{feed_token_digest}-#{non_existing_record_id}"
          set_param(:feed_token, feed_token)
  ...
  describe '#find_user_from_personal_access_token' do
    ...
    it 'returns nil without access token' do
      allow(self).to receive(:access_token).and_return(nil)

      is_expected.to be_nil
    end

    it 'returns user with correct access token' do
      allow(self).to receive(:access_token).and_return(personal_access_token)

      is_expected.to eq(user)
    end

    context 'when access token has no user' do
    ...
    context 'with a personal access token' do
      ...
      context 'when the token is not valid' do
        it 'returns Gitlab::Auth::ExpiredError if token expired', :aggregate_failures do
          ...
          expect(Gitlab::ApplicationContext.current['meta.auth_fail_token_id'])
