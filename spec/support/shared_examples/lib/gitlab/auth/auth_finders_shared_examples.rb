...
RSpec.shared_examples 'Auth::AuthFinders authenticates the user' do
  ...
  shared_examples 'find user from job token' do |without_job_token_allowed|
    let(:current_authenticated_job) { @current_authenticated_job } # rubocop:disable RSpec/InstanceVariable -- The tested module uses an instance var
  ...
  describe '#find_user_from_bearer_token' do
    ...
    context 'with oauth token' do
      ...
      let(:doorkeeper_access_token) do
        Doorkeeper::AccessToken.create!(application_id: application.id, resource_owner_id: user.id, scopes: 'api')
      end
  ...
  describe '#find_user_from_warden' do
    context 'with CSRF token' do
      ...
      context 'with valid credentials' do
        it 'returns the user' do
          set_header('warden', instance_double(Warden::Proxy, authenticate: user))

          expect(find_user_from_warden).to eq identity
        end
      end
    end
    ...
    context 'when the request format is atom' do
      ...
      context 'when old format feed_token param is provided' do
        it 'returns user if valid feed_token' do
          ...
          expect(find_user_from_feed_token(:rss)).to eq identity
        end
      ...
      context 'when path-dependent format feed_token param is provided' do
        let(:feed_token) { "glft-#{OpenSSL::HMAC.hexdigest('SHA256', user.feed_token.to_s, 'url.atom')}-#{user.id}" }
        ...
        it 'returns exception if token has wrong HMAC but same user ID' do
          set_param(:feed_token, "glft-aaaaaaaaaade004d09a968705ba49099229b288b3de43f26c473a9d8d7fb7693-#{user.id}")

          expect { find_user_from_feed_token(:rss) }.to raise_error(Gitlab::Auth::UnauthorizedError)
        end

        it 'returns exception if user does not exist' do
          set_param(:feed_token,
            "glft-a8cc74ccb0de004d09a968705ba49099229b288b3de43f26c473a9d8d7fb7693-#{non_existing_record_id}")
      ...
      context 'when token header param is present' do
        context 'when token is correct' do
          it 'returns the user' do
            ...
            expect(find_user_from_static_object_token(format)).to eq(identity)
          end
        end
  ...
  describe '#find_user_from_access_token' do
    ...
    context 'when validate_access_token! returns valid' do
      it 'returns user' do
        ...
        expect(find_user_from_access_token).to eq identity
      end

      it 'returns exception if token has no user' do
        set_header(described_class::PRIVATE_TOKEN_HEADER, personal_access_token.token)
        allow_next_found_instance_of(PersonalAccessToken) do |instance|
          allow(instance).to receive(:user).and_return(nil)
        end
    ...
    context 'with automatic reuse detection' do
  ...
  describe '#find_user_from_web_access_token' do
    ...
    context 'with no feed, API, archive or download requests' do
    ...
    it 'returns the user for RSS requests' do
      ...
      expect(find_user_from_web_access_token(:rss)).to eq(identity)
    end

    it 'returns the user for ICS requests' do
      set_header('SCRIPT_NAME', 'url.ics')

      expect(find_user_from_web_access_token(:ics)).to eq(identity)
    end

    it 'returns the user for ARCHIVE requests' do
      set_header('SCRIPT_NAME', '/-/archive/main.zip')

      expect(find_user_from_web_access_token(:archive)).to eq(identity)
    end

    it 'returns the user for DOWNLOAD requests' do
      set_header('SCRIPT_NAME', '/-/1.0.0/downloads/main.zip')

      expect(find_user_from_web_access_token(:download)).to eq(identity)
    end

    context 'with for API requests' do
      it 'returns the user' do
        set_header('SCRIPT_NAME', '/api/endpoint')

        expect(find_user_from_web_access_token(:api)).to eq(identity)
      end
      ...
      context 'when the token has read_api scope' do
        let_it_be(:personal_access_token, freeze: true) do
          create(:personal_access_token, user: user, scopes: ['read_api'])
        end
        ...
        it 'finds the user when the read_api scope is passed' do
          expect(find_user_from_web_access_token(:api, scopes: [:api, :read_api])).to eq(identity)
        end
      end
  ...
  describe '#find_personal_access_token' do
    ...
    context 'when passed as header' do
    ...
    context 'when passed as param' do
      it 'returns token if valid personal_access_token' do
  ...
  describe '#find_personal_access_token_from_http_basic_auth' do
    ...
    context 'when access token is not valid' do
      let(:route_authentication_setting) { { basic_auth_personal_access_token: true } }
    ...
    context 'when route_setting is not set' do
      it 'returns nil' do
        auth_header_with(personal_access_token.token)
    ...
    context 'when route_setting is not correct' do
  ...
  describe '#find_user_from_basic_auth_password' do
    ...
    it 'returns nil with CI username XXXXXXX' do
  ...
  describe '#find_user_from_personal_access_token' do
    ...
    context 'when personal access token is nil' do
      let_it_be(:personal_access_token) { nil }
    ...
    context 'when personal access token has no user' do
      let_it_be(:personal_access_token) { create(:personal_access_token, user: user) }

      it 'returns exception' do
        allow(personal_access_token).to receive(:user).and_return(nil)
    ...
    context 'with a personal access token' do
      ...
      context 'when the token is not valid' do
        it 'returns Gitlab::Auth::ExpiredError if token expired', :aggregate_failures do
          ...
          expect(
            Gitlab::ApplicationContext.current['meta.auth_fail_token_id']
          ).to eq("PersonalAccessToken/#{personal_access_token.id}")
        end
        ...
        it 'returns Gitlab::Auth::InsufficientScopeError if invalid token scope', :aggregate_failures do
          ...
          end.to raise_error(Gitlab::Auth::InsufficientScopeError)
          expect(request.env).not_to have_key(described_class::API_TOKEN_ENV)
    ...
    context 'for route_authentication_setting[job_token_allowed]' do
      ...
      with_them do
        ...
        context 'when the token is in the headers' do
          ...
          it { is_expected.to eq(expect_user_via_request ? identity : nil) }
        end
        ...
        context 'when the token is in basic auth header' do
          ...
          it { is_expected.to eq(expect_user_via_basic_auth ? identity : nil) }
        end
      end
    end
  ...
  describe '#cluster_agent_token_from_authorization_token' do
    ...
    context 'when route_setting allows cluster agent token' do
      ...
      context 'when Authorization header matches agent token' do
        before do
        ...
        context 'when agent token has been revoked' do
          before do
            agent_token.revoked!
          end
  ...
  describe '#authentication_token_present?' do
    ...
    context 'with no auth header/param/oauth' do
      before do
