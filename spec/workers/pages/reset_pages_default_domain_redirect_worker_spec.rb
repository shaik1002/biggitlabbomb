...
RSpec.describe Pages::ResetPagesDefaultDomainRedirectWorker, feature_category: :pages do
  ...
  let(:default_domain_url) { 'default.domain.com' }
  let(:non_default_domain_url) { 'non-default.domain.com' }
  let!(:default_domain) { create(:pages_domain, project: project, domain: default_domain_url) }
  let!(:non_default_domain) { create(:pages_domain, project: project, domain: non_default_domain_url) }
  ...
  context 'when the removed domain is not the default domain' do
    let(:event) do
      ::PagesDomains::PagesDomainDeletedEvent.new(data: {
        ...
        domain: non_default_domain.domain
      })
    end
