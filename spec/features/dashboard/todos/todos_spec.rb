...
RSpec.describe 'Dashboard Todos (Vue version)', :js, feature_category: :notifications do
  ...
  describe 'sorting' do
    let!(:oldest_but_most_recently_updated) { create_todo(created_at: 3.days.ago, updated_at: 3.minutes.ago, target: issue) }
    let!(:middle_old_and_middle_updated) { create_todo(created_at: 2.days.ago, updated_at: 2.hours.ago, target: issue2) }
    let!(:newest_but_never_updated) { create_todo(created_at: 1.day.ago, updated_at: 1.day.ago, target: issue3) }
  ...
  describe 'filtering' do
    let!(:self_assigned) { create_todo(author: user, target: issue) }
    let!(:self_marked) { create_todo(author: user, target: issue2, action: :marked) }
    let!(:other_assigned) { create_todo(author: user2, target: issue3) }
