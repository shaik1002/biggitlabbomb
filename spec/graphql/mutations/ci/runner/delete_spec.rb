...
RSpec.describe Mutations::Ci::Runner::Delete, feature_category: :runner do
  ...
  describe '#resolve' do
    ...
    context 'when user can delete owned runner' do
      ...
      let!(:project) { create(:project, creator_id: user.id, maintainers: [user]) }
    ...
    def setup_service_expectations(runner)
      expect_next_instance_of(
        ::Ci::Runners::UnregisterRunnerService, runner, current_ctx[:current_user],
        { caller: 'GraphQL RunnerDelete mutation' }
      ) do |service|
