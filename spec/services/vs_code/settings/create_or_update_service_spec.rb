...
RSpec.describe VsCode::Settings::CreateOrUpdateService, feature_category: :web_ide do
  describe '#execute' do
    ...
    context 'when setting_type is extensions' do
      ...
      subject do
        described_class.new(current_user: user, settings_context_hash: settings_context_hash,
      ...
      it 'creates a new record when a record with settings_context_hash does not exist' do
        ...
        record = User.find(user.id).vscode_settings.by_setting_types(['extensions'], settings_context_hash).first
        ...
        described_class.new(current_user: user,
          settings_context_hash: new_settings_context_hash, params: extensions_params).execute
      ...
      it 'updates record if a record with the same setings_context_hash exists' do
        setting = create(:vscode_setting, user: user, setting_type: 'extensions',
          settings_context_hash: settings_context_hash)

        expect { subject }.to change {
          setting.reload.content
        }.from(setting.content).to(extensions_params[:content]).and not_change { setting.reload.uuid }
      end
    end
