...
RSpec.describe ::Packages::Npm::GenerateMetadataForNamespaceService, feature_category: :package_registry do
  ...
  let(:metadata) { described_class.new(package_name, [], { current_user: user, group_or_namespace: group }).execute }
  ...
  describe '#versions' do
    ...
    subject(:versions) { metadata[:versions] }
    ...
    with_them do
      ...
      it { expect(versions[package1.version].with_indifferent_access).to match_schema(version_schema) }
      it { expect(versions[package2.version].with_indifferent_access).to match_schema(version_schema) }
      it { expect(versions[package1.version]['custom_field']).to be_blank }

      context 'for dependencies' do
        ::Packages::DependencyLink.dependency_types.each_key do |dependency_type|
          if params[:has_dependencies]
            it { expect(versions.dig(package1.version, dependency_type.to_s)).to be_any }
          else
            it { expect(versions.dig(package1.version, dependency_type)).to be_nil }
          end

          it { expect(versions.dig(package2.version, dependency_type)).to be_nil }
        end
      ...
      context 'for metadatum' do
        ::Packages::Npm::GenerateMetadataService::PACKAGE_JSON_ALLOWED_FIELDS.each do |metadata_field|
          if params[:has_metadatum]
            it { expect(versions.dig(package1.version, metadata_field)).not_to be_nil }
          else
            it { expect(versions.dig(package1.version, metadata_field)).to be_nil }
          end

          it { expect(versions.dig(package2.version, metadata_field)).to be_nil }
        end
      end

      # rubocop:disable RSpec/NoExpectationExample -- expectations are in the check_n_plus_one method
    ...
    context 'with package files pending destruction' do
      ...
      let(:shasums) { versions.values.map { |v| v.dig(:dist, :shasum) } }
  ...
  describe '#dist_tags' do
    subject(:dist_tags) { metadata[:dist_tags] }

    context 'for packages without tags' do
      it { is_expected.to be_a(Hash) }
      it { expect(dist_tags['latest']).to eq(latest_package.version) }
    ...
    context 'for packages with tags' do
      ...
      it { is_expected.to be_a(Hash) }
      it { expect(dist_tags[package_tag1.name]).to eq(package1.version) }
      it { expect(dist_tags[package_tag2.name]).to eq(package1.version) }
      it { expect(dist_tags[package_tag3.name]).to eq(package2.version) }
      it { expect(dist_tags[package_tag4.name]).to eq(latest_package.version) }
      it { expect(dist_tags[package_tag5.name]).to eq(latest_package.version) }
      ...
      context 'with duplicate tags' do
        context 'when in different projects' do
          ...
          it "returns the tag of the latest package's version" do
            expect(dist_tags['latest']).to eq(package2.version)
          end
        end
        ...
        context 'when in the same project' do
          ...
          it "returns the most recent tagged package's version" do
            expect(dist_tags['tag']).to eq(package_b.version)
          end
        end
      end
      ...
      context 'when fetching all package tags' do
        ...
        it 'returns all tags' do
          ...
          expect(dist_tags.size).to eq(Packages::Tag.count)
        end
      end
    end
  end
  ...
  context 'when passing only_dist_tags: true' do
    subject(:dist_tags) do
      described_class.new(package_name, [], { current_user: user, group_or_namespace: group })
        .execute(only_dist_tags: true)
    end

    it 'returns only dist tags' do
      expect(dist_tags.payload.keys).to contain_exactly(:dist_tags)
    end
  end
