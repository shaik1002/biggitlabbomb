...
RSpec.describe WebHook, feature_category: :webhooks do
  ...
  describe '#backoff!' do
    ...
    context 'when hook is at the failure threshold' do
      ...
      context 'when hook is next told to backoff' do
        ...
        it 'causes the hook to become disabled for initial backoff period' do
          ...
          expect(hook).to have_attributes(
            recent_failures: WebHooks::AutoDisabling::EXCEEDED_FAILURE_THRESHOLD,
            backoff_count: 1,
    ...
    context 'when the counters are above EXCEEDED_FAILURE_THRESHOLD' do
      let(:threshold) { WebHooks::AutoDisabling::EXCEEDED_FAILURE_THRESHOLD }

      before do
        hook.update!(
          recent_failures: (threshold + 1),
          backoff_count: (threshold + 1),
          disabled_until: 1.hour.ago
        )
      end

      it 'reduces the counters to EXCEEDED_FAILURE_THRESHOLD' do
        hook.backoff!

        expect(hook).to have_attributes(
          recent_failures: threshold,
          backoff_count: threshold
        )
      end
    end
  end
  ...
  describe '#failed!' do
    ...
    context 'when the recent failure value is above EXCEEDED_FAILURE_THRESHOLD' do
      let(:threshold) { WebHooks::AutoDisabling::EXCEEDED_FAILURE_THRESHOLD }

      before do
        hook.update!(recent_failures: (threshold + 1))

        # Note this scenario is technically not possible as #failed! is a no-op once the hook
        # has already been disabled. But to allow the test, stub the hook so we can call #failed! again
        allow(hook).to receive(:permanently_disabled?).and_return(false)
      end

      it 'reduces to EXCEEDED_FAILURE_THRESHOLD' do
        hook.failed!

        expect(hook.recent_failures).to eq(threshold)
      end
    end
  end
end
