...
RSpec.describe Gitlab::ImportExport::Json::StreamingSerializer, :clean_gitlab_redis_shared_state, feature_category: :importers do
  ...
  let(:cache_user_contributions) { true }
  ...
  subject do
    described_class.new(exportable, relations_schema, json_writer, exportable_path: exportable_path, logger: logger, current_user: user, cache_user_contributions: cache_user_contributions)
  end
  ...
  describe '#execute' do
    ...
    context 'with many relations' do
      ...
      context 'contributing user id caching' do
        ...
        let(:expected_user_ref_ids) do
          Issue.all.pluck(:author_id, :updated_by_id, :last_edited_by_id, :closed_by_id)
            .flatten.uniq.filter_map { |user_id| user_id&.to_s }
        end
        ...
        context 'when cache_user_contributions is false' do
          let(:cache_user_contributions) { false }

          it 'does not cache existing referenced user_ids' do
            subject.execute
