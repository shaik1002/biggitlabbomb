...
RSpec.describe Gitlab::BackgroundMigration::AlterWebhookDeletedAuditEvent, feature_category: :webhooks do
  ...
  let!(:audit_event2) do
    audit_events.create!(attributes.merge(id: 2, target_type: 'GroupHook', target_details: "example2@example.com"))
  end

  let!(:audit_event3) do
    audit_events.create!(attributes.merge(id: 3, target_type: 'ProjectHook', target_details: "example3@example.com"))
  end
  ...
  describe '#perform' do
    ...
    it 'alters target details column' do
      ...
      audit_events = AuditEvent.all.first.to_a

      expect(audit_events[0].target_details).to eq("Hook 1")
      expect(audit_events[1].target_details).to eq("Hook 2")
      expect(audit_events[2].target_details).to eq("Hook 3")
