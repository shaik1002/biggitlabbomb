  ...
  describe '#address' do
    ...
    shared_examples 'with incoming email address' do
      context 'when incoming email is enabled' do
        before do
          config = instance_double(::GitlabSettings::Settings, enabled: true, address: 'test+%{key}@mail.com')
      ...
      context 'when incoming email is disabled' do
        before do
          config = instance_double(::GitlabSettings::Settings, enabled: false)
          allow(::Gitlab.config).to receive(:incoming_email).and_return(config)
        end
    ...
    context 'when service_desk_email is enabled' do
      before do
        config = instance_double(::GitlabSettings::Settings, enabled: true, address: 'foo+%{key}@bar.com')
