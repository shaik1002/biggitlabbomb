    ...
    class MetricDefiner
      ...
      def prompt_for_product_categories(defaults)
        assign_shared_attr(:product_categories) do
          ...
          potential_groups = selected_events.map(&:product_group).push(defaults[:product_group])
          prompt_for_feature_category(
      ...
      def create_metric_files
        @metrics.map.with_index do |metric, idx|
          new_page!(10, 10, STEPS) # Repeat the same step number but increment metric counter
      ...
      def collect_values_for_shared_event_properties
        ...
        # Use event value as default if it's the same for all selected events
        ...
        # If an event is relevant to a category, then the metric will be too
